...
# T_sw_value = 2e5 (k)
# psi = rho, V, p, B
# epsil = np.sqrt(x ** 2 + y ** 2 + z ** 2)

# bc1 -> dpsi/ddim = 0
# x = -48, y = 24, z = 24

# bc2 -> mirror boundary condition
# z = 0, 
# psi(y, -z) = psi(x, z) for rho, vx, vy, p, bz 
# psi(y, -z) = -psi(x, y) for vz, bx, by
# y = 0,
# psi(-y, z) = psi(y, z) for rho, vx, vz, p, bx, bz
# psi(-y, z) = -psi(y, z) for vy, by

# bc3 -> 


# mu_vis = D_p * rho_sw_value


### data information dataframes
# 2d df
...
### Earth boundary conditions defined at x, y, z = 0
        ...
        if torch.mean(rho_0_value) < 0.2 * torch.mean(rho_earth):
        ...
        if torch.mean(p_0) < torch.mean(p_earth):
            p_0 = p_earth
        ...
        if x_inner_boundary.shape[0] > 1:
            F_inner_boundary = F(x_inner_boundary, params)
            dFiEdX_e = torch.autograd.grad(
                inputs = x_inner_boundary,
                outputs = F_inner_boundary,
                grad_outputs=torch.ones_like(F_inner_boundary),
                retain_graph=True, 
                create_graph=True
            )[0]
            loss_consts = loss(dFiEdX_e, torch.zeros_like(dFiEdX_e))
        else:
            loss_consts = 0
...
### loss final calculation
        physics_loss = new_mc_loss + 2 * new_mo_loss + new_pg_loss + new_fg_loss + Earth_dp_loss
        ...
        return physics_loss, Earth_dp_loss, new_mc_loss, new_mo_loss, new_pg_loss, new_fg_loss
    return loss_fn
